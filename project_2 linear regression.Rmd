...
```{r}
# Use the model to predict prices for the validation dataset
final_predictions <- predict(model1, newdata = valid_data)
...
mean_predicted_price <- mean(final_predictions)
mean_predicted_price
```

*Interpretation*: To create the final prediction, I used my linear regression model to generate predicted home prices for the entire validation dataset. Since our project does not require predicting a brand-new home (and we were not given a new record to evaluate), the average predicted price provides a single, easy-to-understand estimate that reflects the model’s typical output. This summary value helps show the overall pricing level the model expects based on the validation data. Overall, this gives us a straightforward final result using Model 1’s predictions. 
